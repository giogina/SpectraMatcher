        ...
        with dpg.collapsing_header(label=state_plot.name, parent=self.spectra_list_group, before=self.last_inserted_spec_tag, default_open=True) as self.spectrum_headers[state_plot.tag]:
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.group(horizontal=False):
                    offset = int(state_plot.spectrum.zero_zero_transition_energy-abs(ExperimentalSpectrum.get_v00(self.viewmodel.is_emission)))
                    self.spectrum_controls[state_plot.tag]['xshift'] = dpg.add_slider_float(format=f"shift = %.0f {'+' if offset < 0 else '-'} {abs(offset)} cm⁻¹", min_value=-1000, max_value=5000, default_value=state_plot.xshift, callback=lambda s, a, u: self.viewmodel.on_x_drag(a, state_plot.tag, slider=True, update_all=True), width=-66)
