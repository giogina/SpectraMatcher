...
class AttributeController(list):
    ...
    def get_font(self) -> Union[None, int]:
    ...
    def get_height(self) -> Union[float, int]:
        return get_text_size('Tg,y', font=self.get_font())[1]

    def render(self, text: str, parent=0, attributes_group=0, max_text_height: Union[float, int] = -1):
...
class StrEntity(str):
    ...
    def set_attributes(self, attributes: Union[list, Attribute]):
    ...
    def __add__(self, __object: Union[SelfStrEntity, SelfTextEntity]) -> Union[SelfStrEntity, SelfTextEntity]:
    ...
    def get_width(self) -> Union[float, int]:
    ...
    def split(self, sep: Union[str, None] = None, **kwargs) -> list:
    ...
    def render(self, parent=0, attributes_group=0, max_text_height: Union[float, int] = -1):
...
class TextEntity(list):
    def split(self, sep: Union[str, None] = None) -> list:
        _list = []
    ...
    def __add__(_self, __object: Union[StrEntity, SelfTextEntity]) -> SelfTextEntity:
...
class LineEntity(TextEntity):
    ...
    def append(self, __object: Union[TextEntity, StrEntity]) -> None:
        __object.recreate_attributes()
        super().append(__object)
        attributes = __object.get_all_attributes()
        list_attributes: list = self.get_attributes_by_type(attributes, List)  # noqa
    ...
    @staticmethod
    def get_attributes_by_type(attributes: list, type: type) -> list:
        return list(filter(lambda item: isinstance(item, type), attributes))

    @staticmethod
    def remove_duplicates_by_depth(attributes: list) -> list:
        return list({i.depth: i for i in attributes}.values())

    @staticmethod
    def get_width(text_entity: Union[TextEntity, StrEntity]) -> Union[float, int]:  # noqa
        width = text_entity.get_width()
        attributes = text_entity.get_all_attributes()

        blockquote_attributes: list = LineEntity.get_attributes_by_type(attributes, Blockquote)  # noqa
        blockquote_attributes: list = LineEntity.remove_duplicates_by_depth(blockquote_attributes)  # noqa
        for attribute in blockquote_attributes:
            width += attribute.get_width()

        list_attributes: list = LineEntity.get_attributes_by_type(attributes, List)  # noqa
        list_attributes: list = LineEntity.remove_duplicates_by_depth(list_attributes)  # noqa
        for attribute in list_attributes:
            width += attribute.get_width()
        separator_attributes: list = LineEntity.get_attributes_by_type(attributes, Separator)  # noqa
    ...
    def render_attributes(self, text_entity: TextEntity, parent=0, attributes_group=0):
        ...
        blockquote_attributes: list = self.get_attributes_by_type(attributes, Blockquote)  # noqa
        blockquote_attributes: list = self.remove_duplicates_by_depth(blockquote_attributes)  # noqa
        for attribute in blockquote_attributes:
            attribute.render(text_height, parent=parent, attributes_group=attributes_group)

        list_attributes: list = self.get_attributes_by_type(attributes, List)  # noqa
        list_attributes: list = self.remove_duplicates_by_depth(list_attributes)  # noqa
        for attribute in list_attributes:
            attribute.render(text_height, parent=parent, attributes_group=attributes_group)
            self.post_render_queue.append(attribute)

        pre_attributes: list = self.get_attributes_by_type(attributes, Pre)  # noqa
