...
def get_text_size(text: str, *, wrap_width: float = -1.0, font: Union[int, str] = 0, **kwargs) -> Union[List[Union[float, int]], Tuple[Union[float, int], ...]]:
...
def wrap_text_entity(text: Union[text_entities.StrEntity, text_entities.TextEntity],
                     width: Union[float, int] = -1) -> text_entities.LineEntity:
    def get_width(str_entity: Union[text_entities.StrEntity, text_entities.TextEntity]) -> Union[float, int]:
        return text_entities.LineEntity.get_width(str_entity)

    def to_words(str_entity: Union[text_entities.StrEntity, text_entities.TextEntity]) -> list:
        words_list = []
        word = None
...
class _ConvertedMessageEntity:
    def __init__(self, entity: parser.MessageEntity):
        ...
        if type(self.entity) == parser.MessageEntitySeparator:
    ...
    @property
    def object(self):
        if type(self.entity) == parser.MessageEntityBold:
            return font_attributes.Bold
        elif type(self.entity) == parser.MessageEntityItalic:
            return font_attributes.Italic
        elif type(self.entity) == parser.MessageEntityUnderline:
            return text_attributes.Underline
        elif type(self.entity) == parser.MessageEntityStrike:
            return text_attributes.Strike
        elif type(self.entity) == parser.MessageEntityCode:
            return text_attributes.Code
        elif type(self.entity) == parser.MessageEntityPre:
            return text_attributes.Pre(attribute_connector=self.entity.attribute_connector)
        elif type(self.entity) == parser.MessageEntityTextUrl:
            return text_attributes.Url(self.entity.url,
                                       attribute_connector=self.entity.attribute_connector)
        elif type(self.entity) == parser.MessageEntityFont:
            return font_attributes.Font(self.entity.color,
                                        self.entity.size)
        elif type(self.entity) == parser.MessageEntityBlockquote:
            return line_atributes.Blockquote(self.entity.depth,
                                             attribute_connector=self.entity.attribute_connector)
        elif type(self.entity) == parser.MessageEntityUnorderedList:
            return line_atributes.List(self.entity.depth,
                                       attribute_connector=self.entity.attribute_connector,
                                       task=self.entity.task,
                                       task_done=self.entity.task_done)
        elif type(self.entity) == parser.MessageEntityOrderedList:
            return line_atributes.List(self.entity.depth,
                                       attribute_connector=self.entity.attribute_connector,
                                       ordered=True,
                                       ...
                                       task=self.entity.task,
                                       task_done=self.entity.task_done)
        elif type(self.entity) == parser.MessageEntitySeparator:
            return line_atributes.Separator
        elif type(self.entity) == parser.MessageEntityH1:
            return font_attributes.H1
        elif type(self.entity) == parser.MessageEntityH2:
            return font_attributes.H2
        elif type(self.entity) == parser.MessageEntityH3:
            return font_attributes.H3
        elif type(self.entity) == parser.MessageEntityH4:
            return font_attributes.H4
        elif type(self.entity) == parser.MessageEntityH5:
            return font_attributes.H5
        elif type(self.entity) == parser.MessageEntityH6:
...
class MarkdownText:
    text_entity: Union[text_entities.TextEntity, text_entities.StrEntity]
    ...
    def add(self, wrap: Union[float, int] = -1, parent=0):
        '''
...
def add_text(markdown_text: str,
             wrap: Union[float, int] = -1,
             parent: Union[float, str] = 0,
             pos: Union[list, tuple] = None, ) -> int:
    ''' Adds Markdown text.
    :param wrap: Number of pixels from the start of the item until wrapping starts.
    :param parent: Parent to add this item to. (runtime adding)
...
def add_text_italic(default_value: str = '', *, label: str = None, user_data: Any = None,
                    use_internal_label: bool = True, tag: Union[int, str] = 0, indent: int = -1,
                    parent: Union[int, str] = 0, before: Union[int, str] = 0, source: Union[int, str] = 0,
                    payload_type: str = '$$DPG_PAYLOAD', drag_callback: Callable = None, drop_callback: Callable = None,
                    show: bool = True, pos: Union[List[int], Tuple[int, ...]] = [], filter_key: str = '',
                    tracked: bool = False, track_offset: float = 0.5, wrap: int = -1, bullet: bool = False,
                    color: Union[List[int], Tuple[int, ...]] = (-255, 0, 0, 255), show_label: bool = False, **kwargs) -> \
Union[int, str]:
	...
	Returns:
		Union[int, str]
	"""
    dpg_text = dpg.add_text(default_value, label=label, user_data=user_data, use_internal_label=use_internal_label,
                            tag=tag, indent=indent, parent=parent, before=before, source=source,
                            payload_type=payload_type, drag_callback=drag_callback, drop_callback=drop_callback,
                            show=show, pos=pos, filter_key=filter_key, tracked=tracked, track_offset=track_offset,
                            wrap=wrap, bullet=bullet, color=color, show_label=show_label, **kwargs)
    dpg.bind_item_font(dpg_text,
                       font=font_attributes.Italic.get_font())
    return dpg_text


def add_text_bold(default_value: str = '', *, label: str = None, user_data: Any = None, use_internal_label: bool = True,
                  tag: Union[int, str] = 0, indent: int = -1, parent: Union[int, str] = 0, before: Union[int, str] = 0,
                  source: Union[int, str] = 0, payload_type: str = '$$DPG_PAYLOAD', drag_callback: Callable = None,
                  drop_callback: Callable = None, show: bool = True, pos: Union[List[int], Tuple[int, ...]] = [],
                  filter_key: str = '', tracked: bool = False, track_offset: float = 0.5, wrap: int = -1,
                  bullet: bool = False, color: Union[List[int], Tuple[int, ...]] = (-255, 0, 0, 255),
                  show_label: bool = False, **kwargs) -> Union[int, str]:
...
def add_text_bold_italic(default_value: str = '', *, label: str = None, user_data: Any = None,
                         use_internal_label: bool = True, tag: Union[int, str] = 0, indent: int = -1,
                         parent: Union[int, str] = 0, before: Union[int, str] = 0, source: Union[int, str] = 0,
                         payload_type: str = '$$DPG_PAYLOAD', drag_callback: Callable = None,
                         drop_callback: Callable = None, show: bool = True, pos: Union[List[int], Tuple[int, ...]] = [],
                         filter_key: str = '', tracked: bool = False, track_offset: float = 0.5, wrap: int = -1,
                         bullet: bool = False, color: Union[List[int], Tuple[int, ...]] = (-255, 0, 0, 255),
                         show_label: bool = False, **kwargs) -> Union[int, str]:
